...
app.post(
  ...
  (req, res) => {
    ...
    var index = req.body.phone.indexOf('+')
    if(index<0){
...
app.post(
  ...
  (req, res) => {
    ...
    return md.checkUserName(req, res);
  }
);
    ...
    md.roomList(data).then(async (result) => {
      ...
      if (result.length > 0) {
        await asyncForEach(mmy, async (user) => {
          ...
          console.log(user);
          let percentage = user.charge_percentage;
          console.log("percentage from db", percentage);
...
// for match requests
app.post("/match-request", mw.ensureToken, async (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    if (err) {
      res.sendStatus(403);
    } else {
      ...
      var { fromUser, toUser, betAmount, gameId } = req.body;
      toUser = toUser.trim()
      console.log(toUser, toUser.length)
      var data1 = {
        fromUser,
        toUser,
        betAmount,
      ...
      md.checkuser(toUser).then((data) => {
        if (data.length) {
          md.recordRequest(data1).then((data) => {
          ...
          res
            ...
            .json({
              success: false,
              message: "No user exist with username " + toUser,
            });
        }
      });
    }
...
app.post("/delete-request", mw.ensureToken, async (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    if (err) {
      res.sendStatus(403);
    } else {
      ...
      md.deleteRequest(matchId).then((data) => {
        console.log("this is data", data);
...
app.post("/update-request", mw.ensureToken, async (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    if (err) {
      res.sendStatus(403);
    } else {
      //console.log('data.user_id',data.user_id)
      var { matchId } = req.body;
      md.updateRequest(matchId).then((data) => {
        console.log("this is data", data);
...
app.post("/update-matchRequest", mw.ensureToken, async (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    if (err) {
      res.sendStatus(403);
    } else {
      //console.log('data.user_id',data.user_id)
      var { matchId } = req.body;
      md.updateMatchRequest(matchId).then((data) => {
        console.log("this is data", data);
        res.status(200).json({ success: true, message: "Request updated" });
      });
    }
...
app.post("/match-request-list", mw.ensureToken, async (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    if (err) {
      res.sendStatus(403);
    } else {
      ...
      md.matchRequests(user).then((rows) => {
...
app.post("/challenge", mw.ensureToken, (req, res) => {
  jwt.verify(req.token, process.env.SECRET_KEY, async function (err, data) {
    ...
    let [{ percentage }] = await md.getProfitPercent02(poolid);
    console.log("percentage from db", percentage);
  ...
  (req, res) => {
    jwt.verify(
      ...
      async function (err, userdata) {
        ...
        let [{ percentage }] = await md.getChargePercentage();
        console.log("percentage from db", percentage);
        userdata.chargePercentage = percentage;
        md.createPool(userdata)
          .then(async (result) => {
            if (result.insertId) {
              ...
              console.log(rslt.winning_amount, "this is from me");
              rslt.success = true;
              rslt.poolid = result.insertId;
