...
var myArray = ["Kuku","Mogambo","Shakaal","Kilvish","Shaktimaan","Crime Master Gogo","Keju","Saabu","Bunty","Gaitonde","Gabbar","Thakur","Mr. India","K..K..Kiran","Naagraj","Raja Babu","Babita ji","Bhabhi ji","Daya Bhen","Jethalal","Gully Boy","Baba Ji","Bhaiya","Chulbul Pandey","Foofaaji","Didi","Pappu","Dream Girl","Papa Ki Pari","Hansa","Praful","Manjulika","Baa","Bantai","Jerry","Chhote","Pandit Ji","Bholi Punjaban","Jattani","Choochaa","Thakuraain","Khamakha","Yo Yo Singh","Nandu Sabka Bandhu","Anarkali","Bubbly","Soorma Bhopali","Pushpa","Chutki","Maaya Sarabhai","Chandramukhi","Inspector Choutala","Raani ","Bitti","Paaro","Kalank","Devdas","Jeetu" ,"Sharma Ji Ka Ladka","Deewani","Gutthi","Selfie Queen","Leela" ,"Mastaani","Parjaai Ji","Devsena","Kattappa","Baahubali","Chitti" ,"Anna","Hindustani Bhaau","Raajmata","Shivagaamini","Kabir Singh","Pinky","Chandika","Chhota Bheem","Naina","Ae Shivaani","Daaku Daddy","Pammy Aunty","Sakht Launda","Mausi","Basanti","Shivaaji The Boss","Angoori","Komolikaa","Amma Ji","ACP Pradyuman","Circuit","Pappa Ji","Jassi","Madhubaala","Shehenshah","Heera Lal","Kachra Seth","Hasina","Preeto","Billo Raani","Madam Ji" ,"Majnu Bhai","Maaas","Shetty Bhai", "Jagat Janani","Viru","Vasooli bhai" ,"Chhota Chhatri","Dr.Ghungroo","Rancho","Mussadi Lal"];// Update Redeem Request
var actors = myArray.map(a => JSON.stringify(a)).join();

const RedeemStatus = (req, res) => {
  let { body: { orderId} } = req 
  
    disburst(orderId).then((result)=> {
      res.end(JSON.stringify({'result':{ mssg: 'Redeem Request successfully', success: true,result }}));
    });
}
const updateRedeemStatus = (req, res) => {

  let { body: { id: id, status:status, amount, uid }, session } = req  
   var rs1;
 status =  (status == 1) ? 0:1;
        ...
        db.query(query0, function (err0, result0) {
            if (err0) {
                ...
                if (_result0.length > 0) {
                    var query1 = 'UPDATE redeem_history SET status= "' + status + '" WHERE id = "' + id + '"';
                    ...
                    gratification(phone, amount).then(async(result)=>{
                    ...
                    if(status == 1){
                        ...
                        let renderable = {
                            template: path.join("emails", "users", "withdrawal_success.html"),
                            locals: {host: req.hostname}
                        }
                        
                        let rr1 = await mail(options, renderable)

                        let data01 = {mobile:phone, message:'The withdrawal has been processed. Expect payment within 3-5 business days.'}    
                        let rr01 = await sms(data01);
                      }

                    },(err)=>{
                      console.log('err123',err);
                      res.end(JSON.stringify({'result':{ mssg: 'Redeem Request not successfully', success: false, error:err }}));
                    })     
                    setTimeout(() => {
                      res.end(JSON.stringify({'result':{ mssg: 'Redeem Request successfully', success: true }}));
                    }, 3000);
...
const generateError = function (res, text) {
  return res.end(JSON.stringify({result:{ error: text, success: false , mssg:'Operation Not accomplished'}}));
}
const isUserExists = (id) => {
 
    return db.query('SELECT Count(*) as usercount from users where id = "' + id + '"')
...
const updateDeviceId = (deviceid, user_id) => {
  ...
  let sql = "UPDATE users SET device_id = "+deviceid+"  WHERE id ="+user_id;
   return db.query(sql);    
}

const updateWin = (user_id,poolid,win) => {
  let sql = "UPDATE bet_history SET win = "+win+"  WHERE poolid="+poolid+" and user_id ="+user_id;
   return db.query(sql);    
}

const updatePushId = (push_id, user_id) => {
  let sql = "UPDATE user_details SET push_id = '"+push_id+"' WHERE user_id='"+user_id+"'";
   return db.query(sql,[user_id]);  
}
...
if(data.message && data.push_id){
  ...
  if (data.action == 'approved') {
    console.log('config approved',config);
      await Pushclient.sendMessage(config);
    return db.query(`UPDATE challenges SET status = ? WHERE id = ?`, ['approved', data.id])
}
...
const getNotifications = (fbid) => {
  let sql ="SELECT (SELECT username from users where fb_id = n.sender_id) as senderName, n.* FROM `notifications` n left JOIN users ur on ur.fb_id = n.receiver_id WHERE n.receiver_id = '"+fbid+"'";
  return db.query(sql);
   
 }

const friendRequests = (id) => {
  return db.query('SELECT a.* FROM `relationship` as a where a.user_two_id=' + id + ' and a.status=0');
}

const friends = (id) => {
  let sql = 'SELECT b.id,b.username, a.status FROM `relationship` as a JOIN users as b on (a.user_one_id=b.id and a.user_one_id!=' + id + ') or (a.user_two_id=b.id and a.user_two_id!=' + id + ')  WHERE (a.`user_one_id` = ' + id + ' OR a.`user_two_id` = ' + id + ') AND a.`status` = 1';
  return db.query(sql)
  
}
const getTotalSpent = (id) => {

 let sql ='SELECT SUM(amount) as totalspent from spent_log where user_id = "' + id + '" group by user_id ';
 //console.log(sql)
 return db.query(sql);
...
const spentLogByuser = (id) => {
 ...
 let sql ='SELECT sp.*, us.username from spent_log sp left join users us on sp.user_id=us.id where sp.user_id = "' + id + '" ';
 //console.log(sql)
 return db.query(sql);
...
const isPoolExists = (creator_id,poolid=null) => {
  let myvar;
     ...
     return db.query("SELECT Count(*) as poolcount from pool where creator_id = " + creator_id + " and status = 1"+myvar)
...
const getBetHistory = (game_id=null, user_id) => {
  ...
  let sql = "SELECT bh.bet_amount, p.started, bh.win , DATE_FORMAT(bh.created_at, '%d  %b, %Y %H:%i') AS created from bet_history bh left join pool p on p.id = bh.poolid where p.started = 1 and bh.user_id = ? "+my; 
  //console.log('sql',sql);
  return db.query(sql,[user_id]);
...
const getChallenge = (game_id=null, sender) => {
  ...
  let sql = "SELECT id, status, poolid, sender, receiver, win_amount, bet_amount, end, DATE_FORMAT(created_time, '%d  %b, %Y %H:%i') AS created from challenges where  end = 0 and sender = ? "+my; 
...
const receivedChallenge = (game_id=null, receiver) => {
  ...
  let sql = "SELECT id, status, poolid, sender, receiver, win_amount, bet_amount, end, DATE_FORMAT(created_time, '%d  %b, %Y %H:%i') AS created from challenges where end = 0 and receiver = ? "+my; 
...
const getUserStats = (user_id) => {
 ...
 return db.query('Select SUM(ps.battles_won) as battles_won, SUM(ps.battles_lost) as battles_lost, SUM(ps.coins_won) as coins_won, SUM(ps.coins_lost) as coins_lost, SUM(ps.battles_played) as battles_played from player_statistics ps where ps.user_id = ? group by ps.user_id ',[user_id]);
...
const setCommission = (profit_percent) => {
    ...
    return db.query("UPDATE users SET profit_percent = ? WHERE id = ?" ,[profit_percent, 1]);            
}

const checkpool = (poolid) => {
 
    return db.query('SELECT COUNT(*) as poolCount, started, participants, bet_amount from pool WHERE id = ? and status = 1 GROUP BY id', [poolid])
      .then( async (result01)=> {
...
const userlist = (req, res) => {
    ...
    return db.query("Select COALESCE((select SUM(amount) from spent_log WHERE user_id=us.id GROUP BY user_id),0) as total_spent, ps.coins_won, us.*,DATE_FORMAT(us.created_at, '%d  %b, %Y %H:%i') AS created, ud.phone, ud.coins, ud.location from users us LEFT JOIN user_details ud on us.id = ud.user_id left join player_statistics ps  on us.id = ps.user_id  WHERE us.isadmin != '1' and username NOT IN ("+actors.toString()+") order by us.created_at desc")
}
...
const updateTable = (data) => {
  ...
  let sql = "UPDATE pooltables SET name = ?, bet = ?, percentage = ?, tabletype = ?, targetlines = ? where id=?"; 
    return db.query(sql,[data.name, data.bet, data.percentage,data.tabletype, data.targetlines, data.id]);
 }

const kycRequest = (req, res) => {
    
  return db.query("Select ua.*, us.username, DATE_FORMAT(ua.created_at, '%d  %b, %Y %H:%i') AS created from user_account ua LEFT JOIN users us on us.id = ua.user_id WHERE us.isadmin != '1' and username NOT IN ("+actors.toString()+")");
       
}

const redeemRequest = (req, res) => {
    
  return db.query("Select ua.*, us.username, us.id as uid, DATE_FORMAT(ua.activity_date, '%d  %b, %Y %H:%i') AS created from redeem_history ua LEFT JOIN users us on us.id = ua.user_id WHERE us.isadmin != '1' and username NOT IN ("+actors.toString()+")");
...
const getBetAmount = (poolid, creator_id) => {
    ...
    sql = 'SELECT `participants`->"$.player_'+creator_id+'.bet" as bet FROM pool WHERE id = "'+poolid+'"';
...
const getUsersPerPool = (bet,status) => {
  ...
  sql = 'SELECT COUNT(*) as cnt, p.bet_amount, pt.name, pt.percentage, (2*COUNT(*)*p.bet_amount * (pt.percentage)/100 ) as profit FROM pool p left JOIN pooltables pt on p.bet_amount = pt.bet '+WHERE+' GROUP BY p.bet_amount,pt.name'
...
const getProfitPercent = (poolid) => {
    ...
    sql = "SELECT p.bet_amount as bet, pt.percentage from pool p left join pooltables pt on p.bet_amount = pt.bet where p.id ="+poolid;
   //console.log(sql);
    return db.query(sql);   
}
...
const getPaymentHistory = () => {
  ...
  return db.query("Select us.id, us.username, ph.* from payment_history ph left join users us  on us.id = ph.user_id  WHERE us.isadmin != '1' and username NOT IN ("+actors.toString()+")");
...
const addRedeemHistory = (data) => {
  return getUserCoins(data.user_id).then(async(result)=> {
        ...
        if(result[0].win_amount >= data.redeem_amount){
          ...
          return  db.query('INSERT INTO redeem_history SET ?', Object.assign({}, data));
...
const updatePaymentHistory = (amount, trans_id, payment_status, respcode, respmsg, order_id) => {
 let sql = "UPDATE payment_history SET amount = ?, trans_id = ?, payment_status = ?, respcode = ?, respmsg = ? WHERE order_id = ?";
    //console.log(sql);
   return db.query(sql,[amount, trans_id, payment_status, respcode, respmsg, order_id]);
    
}

const updateUserBalance = (user_id, amount, oper) => {

 let sql = "UPDATE user_details SET coins = IF(coins >= 0, coins "+oper+" "+amount+", 0) WHERE user_id = ?";
    //console.log(sql);
   return db.query(sql,[user_id]);
...
const updateUserWinBalance = (user_id, amount, oper) => {
  ...
  let sql = "UPDATE user_details SET win_amount = IF(win_amount >= 0, win_amount "+oper+" "+amount+", 0) WHERE user_id = ?";
    return db.query(sql,[user_id]);   
 }

const updateUserStat = (user_id, game_id, field, amount, oper) => {

    return db.query("select id from player_statistics where user_id =? and game_id=? ",[user_id, game_id])
          .then((result)=>{
            if(result.length > 0){
              return db.query("UPDATE player_statistics SET "+field+" = IF("+field+" >= 0, "+field+" "+oper+" "+amount+", 0) WHERE user_id =? and game_id=? ",[user_id, game_id]);   
            }else{
              sql = 'INSERT INTO player_statistics (user_id,game_id, '+field+') values('+user_id+','+game_id+', '+amount+')';
              console.log(sql);
              return  db.query(sql);
            }
          })
 ...
 const updateUserStatTogether = (user_id, data) => {
  ...
  return db.query("select id from player_statistics where user_id =? ",[user_id])
        .then((result)=>{
          if(result.length > 0){ console.log('data.length:',Object.keys(data).length);
            if(Object.keys(data).length == 4){
              let sql = "UPDATE player_statistics SET battles_won = battles_won + "+data.battles_won+", coins_won = coins_won + "+data.coins_won+", coins_lost = coins_lost +"+data.coins_lost+", battles_played = battles_played + "+data.battles_played+"  WHERE user_id = "+user_id
              console.log(sql);
              return db.query(sql);   
            }else{
              let sql = "UPDATE player_statistics SET battles_lost = battles_lost + "+ data.battles_lost+", coins_lost= coins_lost +"+data.coins_lost+", battles_played = battles_played + "+data.battles_played+"  WHERE user_id = "+user_id
              console.log(sql);
              return db.query(sql);   
            }
            
          }else{
            return db.query('INSERT INTO player_statistics SET ?', Object.assign({}, data));
...
const updateUsername = (username, avatar, user_id) => {
  ...
  if(avatar){
   ...
   return db.query("UPDATE users SET username = ?  "+avatar+"  WHERE id = ?",[username, user_id]);
    
}

const updateUserAvatar = (user_id, base64_str) => {

   return db.query("UPDATE users SET avatar = ?  WHERE id = ?",[base64_str, user_id]);
    
}

const updateOffer = (name, base64_str, offer_percentage) => {

  return db.query("UPDATE offers SET name = ?, image = ?, offer_percentage = ? WHERE id = ?",[name, base64_str, offer_percentage, 1]);
   
}

const updateLevel = (level) => {

  return db.query("UPDATE ai_levels SET difficulty_level = ?  WHERE id = ?",[level, 1]);
...
const updateCue = (user_id, cue_id) => {
  ...
  var cue, data;
       return db.query('SELECT cue from user_details WHERE user_id = ?', [user_id])
            .then((result) => {
               cue = result[0].cue;
              if(cue == null){
                data = cue_id;
...
const updateChat = (user_id, chat_id) => {
  var cue, data;
       return db.query('SELECT chat from user_details WHERE user_id = ?', [user_id])
            .then((result) => {
               chat = result[0].chat;
              if(chat == null){
                data = chat_id;
...
const setCurrentChat = (user_id, chat_id) => {
  ...
  return db.query("UPDATE user_details SET current_chat = ? WHERE user_id = ?" ,[chat_id, user_id]);  
               }else{
...
const setCurrentCue = (user_id, cue_id) => {
  ...
  return db.query("UPDATE user_details SET current_cue = ? WHERE user_id = ?" ,[cue_id, user_id]);  
               }else{
                return {msg:"Please purchase this item first."}
               }
        }
...
const buyCue = (req, res, userdata) => {
   ...
   var totalCoins,rs1,rs2,rs3, finalresult={};
      
     db.query('SELECT coins from user_details WHERE user_id = ?', [userdata.user_id])
            .then(async (result) => {
              ...
              let totalCoins = result[0].coins;
               ...
               let coins_left = totalCoins - rs[0].price;
                ...
                if ( rs[0].price > totalCoins) {
                    res.json({result:{ success:false, error: "You do not have sufficient balance" }});
            ...
            .then((newuser01) => {
               if(newuser01 == false){return false;}           
...
const signup = (req, res) => {
   ...
   let { body: { device_id, pin, gender, username, dob, email, password, phone, avatar, push_id} } = req
   var user, newUser, newUserDetail, user_id = '', player_stat;
      ...
      db.query("SELECT COUNT(*) as userCount, email, phone, username from users WHERE email = ? or username = ? or phone = ? GROUP BY email, username, phone", [email,username,phone])
            .then((result) => {
              //console.log('signup result',result);
                ...
                if (result.length > 0) {
                  if(user.email == email)
                    return res.json({result:{ success:false, error: "Email already exists!" }});
                  if(user.username == username)
                    return res.json({result:{ success:false, error: "Username already exists!" }});
                  if(user.phone == phone)
                    return res.json({result:{ success:false, error: "Phone Number already exists!" }});
                    return false;
                } else {
            ...
            .then( async (newuser01) => {
               ...
               if(newuser01.affectedRows){
                ...
                if (req.originalUrl.indexOf("api") != -1) {
                    var token = jwt.sign({ user_id: newuser01.user_id, username: username }, process.env.SECRET_KEY);
                    newUser.token = token
                }
                newUser.avatar = avatar;

             client.verify.services(sv)
              .verifications
              .create({to:phone, channel: 'sms'})
              .then((verification) =>{ console.log('verification',verification);
               if(verification.status == 'pending'){
                ...
                if(err.code == 60200){
                  ...
                  return res.json({ result:newUser})
                   ...
                   return res.json({result:{ msg:"Couldn't save new user", success:false }}); 
                }
            })
}
...
const facebookSignup = (req, res) => {
    let { session, body:{fb_id,location, username, email, device_id,game_id, push_id} } = req
    var result = {}, player_stat, user_id, rslt = {};

    db.query("SELECT id, email, avatar, device_id, username from users WHERE fb_id = '"+fb_id+"' ")
        .then((rows) => {
          
            if (rows.length > 0) {
                user = rows[0];
           /*  if (user.device_id != null && user.device_id != device_id ) { 
                  res.json({result:{ error: "Login failed\n You are logged in somewhere else. ", mssg: "Login failed\n You are logged in somewhere else." }})
                  return false;
              } */
        ...
        .then(async (rslt) => { console.log('rslt:',rslt);
          if(rslt == false){
       ...
       let rs = await getUserDetails(result.id);
          ...
          var token = jwt.sign({ user_id: result.id, username: session.username, coins: rs[0].coins }, process.env.SECRET_KEY);
          ...
          // }
            ...
            return res.json({result:{friends:friendlist,coins: rs[0].coins, avatar:result.avatar, isadmin: session.usertype, success: true, username: session.username, token: token, user_id: result.id, setCue: rs[0].current_cue, setChat: rs[0].current_chat, chat, cue, user_dp: rs[0].user_dp }})
        })
...
const guestLogin = (req, res) => {
    ...
    let { session, body:{device_id, location, push_id} } = req
    var result = {}, player_stat, user_id;

    db.query('SELECT id, username, avatar from users WHERE device_id = ?', [device_id])
        .then((rows) => {
       ...
       let rs = await getUserDetails(result.id);
          ...
          var token = jwt.sign({ user_id: result.id, username: session.username, coins: rs[0].coins, user_dp: rs[0].user_dp }, process.env.SECRET_KEY);
          ...
          if(ckUser.length >0){
            ...
            return res.json({result:{friends:friendlist, coins: rs[0].coins, avatar:result.avatar, isadmin: session.usertype, success: true, username: session.username, token: token, user_id: result.id, setCue: rs[0].current_cue, setChat: rs[0].current_chat, chat, cue }})
          }else{
            return res.json({result:{ success: false}})
          }
        })
...
const checkUserInPool = (user_id, started=null,logincheck=null,poolid=null) => {
  let mywhere;
  ...
  sql = 'SELECT id as poolid, JSON_KEYS(participants) as kys, JSON_UNQUOTE(JSON_EXTRACT(participants, "$.player_'+user_id+'.user_id")) AS userid FROM pool WHERE JSON_EXTRACT(participants, "$.player_'+user_id+'.user_id") = '+user_id+' and status=1'+mywhere;
   //console.log(sql);
  return db.query(sql);
...
const checkPoolFor = (user_id) => {
  ...
  sql = 'SELECT id as poolid,game_id, JSON_KEYS(participants) as kys, JSON_UNQUOTE(JSON_EXTRACT(participants, "$.player_'+user_id+'.user_id")) AS userid FROM pool WHERE JSON_EXTRACT(participants, "$.player_'+user_id+'.user_id") = '+user_id+' and status = 1 and started=1';
  // console.log(sql);
  return db.query(sql);
...
const botLogin = (req, res) => {
  var result = {}, player_stat, user_id, pc, difference;
  let sql = 'SELECT id FROM `users` WHERE username IN ('+actors.toString()+')'; 
  return db.query(sql)
            .then( async (result)=> { 
              if(result.length > 0){
                 ...
                 difference = result01.filter(x => !pc.includes(x)).concat(pc.filter(x => !result01.includes(x)));
              }
     ...
     let rs = await getUserDetails(result.id);
        ...
        let friendlist = rows.map(function (row) {
         ...
         let [{username}] = await getUserUsername(result.id);
          ...
          return res.json({result:{difficulty_level, username, user_id:result.id,avatar:avatar, success: true, setCue: rs[0].current_cue, setChat: rs[0].current_chat, chat, cue, user_dp: rs[0].user_dp }})
        }else{
          return res.json({result:{ success: false}})
        }
      })
...
const login = (req, res) => {
    P.coroutine(function* () {
        let { body: { email: email, password: rpassword, pin: rpin, push_id, device_id}, session } = req
        let sql = 'SELECT COUNT(id) as userCount, username, device_id, email, id, pin, password, status, isadmin, phone_verify from users WHERE email = "'+ email+'" or username = "'+email+'" or phone = "'+email+'" and pin IS NOT NULL GROUP BY id LIMIT 1';
          //console.log(sql)
        let user = yield db.query(sql)
            ...
            if (user.length) {
              let Pool = yield checkUserInPool(user[0].id);
                ...
                if (user[0].status == 0 ) { 
                    res.json({result:{ error: "User inactive. Please contact administrator!", mssg: "User inactive. Please contact administrator!" }})
                    return;
                }

                if (user[0].phone_verify == 0 ) { 
                  res.json({result:{ error: "Your Phone is not verified!", mssg: "Your Phone is not verified!" }})
                  return;
              }

                let [{ userCount, id, password, username, email, isadmin, gender, pin }] = user
                var same
                if (!req.body.exist) {
           ...
           /*  if (user[0].device_id != null && user[0].device_id != device_id && user[0].id != 2) { 
                  res.json({result:{ error: "Login failed\n You are logged in somewhere else. ", mssg: "Login failed\n You are logged in somewhere else." }})
                  return;
              } */

            if (!same) { 
                    return res.json({result:{ error: "Wrong Pin!", mssg: "Wrong Pin!" }})
                } else {
                    ...
                    var token = jwt.sign({ user_id: id, username: setedUser.username, coins: rs[0].coins }, process.env.SECRET_KEY);
                    ...
                    let ckUser = yield checkUserInPool(id,0,logincheck=1);                                   
                      ...
                      return res.json({result:{friends:friendlist, isadmin: setedUser.usertype, success: true, username: setedUser.username, coins: rs[0].coins, token: token, user_id: setedUser.id, setCue: rs[0].current_cue, setChat: rs[0].current_chat, chat, cue, avatar, user_dp: rs[0].user_dp }})
                      } else { 
                      return res.json({result:{isadmin: setedUser.usertype, success: true, usertype: setedUser.usertype, user_id: setedUser.id }})
                        }

                    } else {
                        return res.json({result:{ error: 'something wrong', mssg: 'something wrong' }});
                    }
                }
            } else {
                return res.json({result:{ error: "Wrong Email/Phone/Username", mssg: "Wrong Email/Phone/Username" }});
            }
    })()
}
/**
...
const checkInstall = (req, res) => {
  ...
  let { body:{device_id} } = req
  
  db.query("SELECT COUNT(id) as userCount from installs WHERE device_id = ?", [device_id])
      .then(async (rows) => {
...
const checkField = (fields,tbl,where) => {
  ...
  db.query("SELECT "+fields+" from "+tbl+"+ where +", [device_id])
      .then(async (rows) => {
...
const addKyc = (data) => { //console.log('data',data);
    var pd, bd;
   return db.query('SELECT id, pan_doc, bank_doc from user_account WHERE user_id = ?', [data.user_id])
        .then((result)=>{
          if(result.length > 0){
            ...
            return db.query('UPDATE `user_account` SET `full_name` = ?, bank_name = ? , acc_num = ?, ifsc_code=?, pan_num=?, pan_doc=?, bank_doc=?  WHERE  user_id=?', [data.full_name, data.bank_name, data.acc_num, data.ifsc_code, data.pan_num,pd,bd, data.user_id]);
          }else{
            return db.query('INSERT INTO user_account SET ?', Object.assign({}, data));
          }
...
const roomList = (data) => {
   ...
   return db.query("SELECT * from pool WHERE status = 1 and challenge = 0 and started = 0 and created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)")     
      }

const roomList01 = (data) => {
let sql = "SELECT p.* from pool p left join challenges c on c.poolid = p.id WHERE (c.sender ='"+data.username+"' or c.receiver='"+data.username+"') and p.status = 1 and p.challenge = 1 and p.started = 0 and p.created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR)";
    //console.log('sql112233:',sql);  
return db.query(sql);     
      }

const getStarted = (poolid) => { 
    return db.query("SELECT started from pool WHERE id="+poolid+" and started = 0 ")     
    }
...
const joinPool = (data) => {
  let creator_id;
   return db.query('SELECT COUNT(*) as poolCount, bet_amount, participants from pool WHERE id = ? GROUP BY id', [data.poolid])
      .then( async (result01)=> {
        //console.log('result01',result01);
        if(result01.length <= 0){
        ...
        if (result01[0].poolCount > 0 ) {
            let sql = "UPDATE pool SET `participants` = JSON_SET(`participants` ,'$.player"+'_'+data.user_id+"' , JSON_OBJECT('bet',"+result01[0].bet_amount+" , 'user_id', "+data.user_id+")) WHERE `id` = '"+data.poolid+"'";
...
const leavePool = (userId, poolId) => {
  ...
  return db.query("UPDATE pool SET participants = JSON_REMOVE(`participants`, '$.player"+'_'+userId+"') WHERE id=?", [poolId]);
}
...
const deletePool = (creator_id, poolid=null) => {
  ...
  if(poolid){
    ...
    return db.query("DELETE FROM  pool where creator_id = ?"+myvar, [creator_id]);
}
...
const updatePush = (push_id, user_id) => {
  ...
  return db.query(`UPDATE user_details set push_id = ? where user_id = ?`, [push_id, user_id]);
}
...
const createPool = (userdata) => { console.log(userdata);
  var credits;
  //let data = userdata;
  return getUserCredit(userdata.user_id).then( async (rs)=> {
    ...
    let data = {
      ...
      participants:'{"player_'+userdata.user_id+'":{"user_id":"'+userdata.user_id+'","bet":'+userdata.bet+'}}'
    }
...
const commitInner = async (winner_id,looser_id, poolid, game_id ,req) =>{
  ...
  let rs = await getProfitPercent(poolid); 
              ...
              //await updateUserWinBalance(winner_id, (winner_amount - bet) ,'+')
               ...
               let rs12 = await updateUserStat(winner_id, game_id, 'coins_won', coins_won,'+');
...
const Commit = (req, res, userdata) => {
  ...
  let {body:{winner_id, looser_id, poolid, game_id}} = req;
  var rs1 ='';

  return db.query('SELECT Count(*) as usercount from users where id = "' + looser_id + '"')
           .then( async (result01) => {
...
const updateUser = (req, res, data) => {
    ...
    let { body: { userid, username, user_dp, coins, power_ups, no_of_levels, no_of_aircrafts, push_id } } = req  
      ...
      if(user_dp){
        ...
        .catch(function (err) {
         ...
         if(err.code == 'ER_EMPTY_QUERY'){
            res.end(JSON.stringify({'result':{ mssg: 'There is no changes in the update', success: true }}));
         }
    }); 
} 
// FOR DETAILS OF GIVEN USER
...
const updateUserStatus = (req, res) => {
      ...
      let { body: { id: id, status:status }, session } = req  
        ...
        var query0 = 'SELECT username, email from users where id = "' + id + '"';
            ...
            db.query(query0, function (err0, result0) {
                if (err0) {
                    ...
                    var _result0 = JSON.parse(JSON.stringify(result0));
                    if (_result0.length > 0) {
                        var query1 = 'UPDATE users SET status= "' + status + '" WHERE id = "' + id + '"';
                        db.query(query1, function (err1, result1) {
                            if (err1) {
                                ...
                                res.end(JSON.stringify({'result':{ mssg: 'Profile updated successfully', success: true }}));
                                 }
                        });
                    }
                    else {
...
const updateAccountStatus = (req, res) => {
 ...
 status =  (status == 'approved') ? 'disapproved':'approved';
  ...
  //console.log('status:', status);
    var query0 = 'SELECT status, user_id from user_account where id = "' + id + '"';
            ...
            if (err0) {
                ...
                if (_result0.length > 0) {
                  user_id = _result0.user_id;
                    var query1 = 'UPDATE user_account SET status= "' + status + '" WHERE id = "' + id + '"';
                    //console.log(query1)
                    db.query(query1, async function (err1, result1) {
                        if (err1) {
                        ...
                        console.log('rr1',rr1);
                            ...
                            res.end(JSON.stringify({'result':{ mssg: 'Status updated successfully', success: true }}));
                             }
                    });
                }
                else {
...
const forgot = (req, res) => {
  ...
  P.coroutine(function* () {
     ...
     return db.query("SELECT COUNT(*) as userCount, id from users WHERE email = ? and isadmin !='1' group by id", [email])
          .then(row => {
                  ...
                  db.updateToken(row[0].id, req, res)
                      .then(function (result) {
                          if(result.status ==1 && result.data != null){
                            res.json({result:{ mssg: "Email! sent successfully!!", success: true }})
                          }else{
                            res.json({result:{ mssg: "Email! could not be sent", success: false }})
                          }
...
const reset = (req, res) => {
  P.coroutine(function* () {
      ...
      db.query('SELECT user_id, resetPasswordExpires From user_details WHERE resetPasswordToken = "' + token + '"')
          .then(async (result) => {
            //console.log('result',result);
            if(result.length > 0){
            ...
            }else{ console.log('result else');
              res.redirect('/error');
            }
          })
  })()
}
const onlineUsers = () => { return db.query('Select username from users where online="y" and username NOT IN ('+actors.toString()+')');
}
const totalUsers = () => { return db.query('SELECT COUNT(*) userCount,(select COUNT(*) from users where online="y" and username NOT IN ('+actors.toString()+')) as online_users FROM users WHERE username NOT IN ('+actors.toString()+')');
}
const currentMonthUsers = () => { return db.query('SELECT COUNT(*) newUsers FROM users WHERE  username NOT IN ('+actors.toString()+') and MONTH(created_at)=MONTH(CURRENT_DATE()) AND YEAR(created_at)=YEAR(CURRENT_DATE())');
}

const todaysUsers = () => { return db.query('SELECT COUNT(*) todaysUsers FROM users WHERE  username NOT IN ('+actors.toString()+') and DAY(created_at)=DAY(CURRENT_DATE()) AND MONTH(created_at)=MONTH(CURRENT_DATE()) AND YEAR(created_at)=YEAR(CURRENT_DATE())');
}


const last24login = () => { return db.query("SELECT COUNT(*) as last24login from last_activity WHERE status = '1' AND online_date > (NOW() - INTERVAL 24 HOUR)");
}
const gamePerday = () =>{
            return db.query('SELECT COUNT(*) as gamesplayed, sum(p.bet_amount) as total_bet, GROUP_CONCAT(us.username) winners, (2*sum(p.bet_amount)*(pt.percentage/100)) as profit, p.created_at from pool p left JOIN pooltables pt ON p.bet_amount = pt.bet left join wins w on w.poolid = p.id left join users us on us.id = w.winner  WHERE p.status = 0 group by DATE_FORMAT(p.created_at,"%y-%m-%d") and username NOT IN ('+actors.toString()+')')
}
