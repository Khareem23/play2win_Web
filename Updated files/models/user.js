...
const matchMakingData = () => {
  let sql = `select mm.*, u.email as winnerEmail from match_making mm
  left join users u on u.id = mm.winner`;
  //console.log(sql)
  return db.query(sql);
};
...
const getBetHistoryMain = (user_id) => {
  console.log(user_id)
  let sql =
    `SELECT bh.*, g.game_name from bet_history bh 
...
const getBets = () => {
  let sql =
    `SELECT bh.id, bh.poolid,bh.game_id, bh.bet_amount,bh.created_at,bh.updated_at, g.game_name from bet_history bh 
    left JOIN games g on g.id = bh.game_id
    where  bh.user_id = 30 `
  return db.query(sql);
};
...
const recordTrans = (data, userid) => {
  ...
  db.query(sql).then((data1) => {
    if (data1.length) {
      sql = `update  user_transactions set transactions = '${JSON.stringify(data)}' where user_id = '${userid}'`;
      return db.query(sql);
    } else {
...
const login = (req, res) => {
  P.coroutine(function* () {
    ...
    let sql =
      'SELECT COUNT(id) as userCount, username, device_id, email, phone, id, pin, password, status, isadmin, phone_verify from users WHERE email = "' +
      email +
      '" or username = "' +
      email +
      '" or phone = "' +
    ...
    if (user.length) {
      ...
      let [
        { userCount, id, password, username, email, isadmin, gender, pin },
      ] = user;
      var same;
      if (!req.body.exist) {
...
const loginWeb = (req, res) => {
  P.coroutine(function* () {
    ...
    let sql = `SELECT u.username, u.email, u.phone, u.id, u.status, u.isadmin, u.phone_verify, ud.coins from users u
        join user_details ud on u.id = ud.user_id
        WHERE email = '${email}' or username ='${email}' or phone ='${email}' and password = '${password}'  `;
    //console.log(sql)
    let user = yield db.query(sql);
...
const editPoolWeb = (data) => {
      return db.query("update pool set game_id = ?, bet_amount = ? where id = ?", [data.game, data.bet, data.poolid]);
  
};
const editBetHistory = (data) => {
      return db.query("update bet_history set game_id = ?, bet_amount = ? where id = ?", [data.game, data.bet, data.id]);
...
const newcommitInner = async (winner_id, looser, poolid, game_id, req) => {
  ...
  var emails = await getUserMails2(
   winner_id,
   winner_id
  );
